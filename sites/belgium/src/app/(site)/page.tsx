...
const COUNTRY = process.env.PUBLIC_COUNTRY_CODE || "BE";
const SITE_URL = process.env.NEXT_PUBLIC_SITE_URL || "https://esimcard.be";
...
export default async function HomePage() {
  ...
  const headline = homepage?.heroHeadline || "Verbind jezelf, voordat je landt.";
  const subheadline = homepage?.heroSubheadline || "Prepaid eSIM voor België zonder contracten en zonder roamingkosten. QR-code direct per e-mail — actief voor je koffer er is.";
  const ctaText = homepage?.heroCtaText || "Toon tarieven";
  ...
  const stepsList = (Array.isArray(rawSteps) && rawSteps.length > 0) ? rawSteps : [
    { step: "01", title: "Tarif für Belgien wählen", description: "Wähle das passende Datenpaket für deine Reise. Der QR-Code wird sofort per E-Mail geliefert." },
    ...
    <>
      ...
      <section>
        <div className="cta-card">
          <h2 style={{ fontSize: "36px", marginBottom: "20px" }}>Waar wacht je nog op?</h2>
          <a className="btn-gradient" href="#tarife">Nu verbinden</a>
        </div>
      </section>
